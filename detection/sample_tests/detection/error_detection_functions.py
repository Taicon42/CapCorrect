...
def update_error_message(err_message, match, matches, allow_profanity, i):
    ...
    if match['shortMessage'] == "Profanity":
        ...
        err_message += match['shortMessage'] + ", "
        error_counter = 1
    return err_message, error_counter
...
def detect_errors(text, client, profanity_bool):
    ...
    if len(matches) != 0:
        ...
        while i < len(matches):
            ...
            err_message, sentence_error_number = update_error_message(err_message, match, matches, allow_profanity, i)
            error_number_total += sentence_error_number
        ...
        return sequence_switched, end_matches, offset_list, err_message, error_number_total
